...
class Trader:
    def __init__(self, Texp, A, fee, ext_fee,
                 initial_amount=1.0,  # ETH
                 price_data='data/ethusdt-1m.short.json.gz',
                 add_reverse=True,
                 extra_args={}):
        self.Texp = Texp
        self.A = A
        ...
        self.price_data = load_prices(price_data, add_reverse=add_reverse)

        self.emas = []
        ema = self.price_data[0][1]
        ema_t = self.price_data[0][0]
        for t, _, high, low, _, _ in self.price_data:
    ...
    def find_target_price(self, p, is_up=True, new=False):
        if is_up:
            for n in range(self.amm.max_band, self.amm.min_band - 1, -1):
                p_down = self.amm.p_down(n)
                dfee = self.amm.dynamic_fee(n, new=new)
                p_down_ = p_down * (1 + dfee)
                if p > p_down_:
                    p_up = self.amm.p_up(n)
                    p_up_ = p_up * (1 + dfee)
                    # if p >= p_up_:
                    #     return p_up
                    # else:
            ...
            for n in range(self.amm.min_band, self.amm.max_band + 1):
                p_up = self.amm.p_up(n)
                dfee = self.amm.dynamic_fee(n, new=new)
                p_up_ = p_up * (1 - dfee)
                if p < p_up_:
        ...
        if is_up:
            return p * (1 - self.amm.dynamic_fee(self.amm.min_band, new=False))
        else:
    ...
    def next_p(self, price_change_up, price_change_down):
        # Example:
        # price_change_up = 0.05, price_change_down=0.03 - stop if price grows up by 5% or drops by 3%
        pass
...
if __name__ == '__main__':
    trader = Trader(600, 100, 0.006, 0.0005)
